...
{
    public class TokenService<T> : ITokenService<T> where T : ApplicationUser
    {
        ...
        private readonly IIdentityManager<T> _identityManager;
        private readonly IHostEnvironment    _env;
        private readonly JwtConfig           _jwtConfig;

        public TokenService(
            IIdentityManager<T> identityManager,
        ...
        public async Task<AuthResponse> GenerateAuthJwtToken(T user)
        {
        ...
        private async Task<IList<Claim>> GetUserClaims(T user, string token)
        {
            // Roles
            var claims = (await _identityManager.UserManager.GetRolesAsync(user))
                .Select(r => new Claim(AppClaims.Roles, r)).ToList();
